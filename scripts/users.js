...
if (process.argv.length <= 2) {
    console.log("No file provided. Add a file to the end, like for example 'node users.js users.json'");
    process.exit();
}
let now = 0;
process.argv.slice(2).forEach((path) => {
    ...
    const username_length = [1, 31];
    const password_length = [1, 255];
    records.forEach(async (record) => {
        ...
        if (typeof username !== "string" ||
            ...
            throw new Error(`Username must be ${username_length[0]}-${username_length[1]} characters long and only consist of letters, numbers, dashes, and underscores`);
        }
        if (typeof password !== "string" ||
            password.length < password_length[0] ||
            password.length > password_length[1]) {
            throw new Error(`Password must be ${password_length[0]}-${password_length[1]} characters long`);
        }
        ...
        try {
            ...
            now = performance.now();
            db.prepare("INSERT INTO user (id, username, password, name, 	role, score, star, button, input, image, right) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(userId, username, hashedPassword, name, role, 0, 0, 0, 0, "", 0);
